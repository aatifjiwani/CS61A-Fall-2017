...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 8
        return not (self.place.ant is None or not self.place.ant.block_path)
        # END Problem 8
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        # BEGIN Problem 5
        place = self.place
        amount_bees = place.bees
        movement = self.min_range

        for _ in range(self.min_range):
          place = place.entrance
          amount_bees = place.bees

        while len(amount_bees) == 0 and place.entrance is not hive:
          place = place.entrance
          amount_bees = place.bees
          movement += 1


        if self.min_range <= movement <= self.max_range:
...
class NinjaAnt(Ant):
    ...
    implemented = True
    food_cost=5
    block_path = False# Change to True to view in the GUI
    # END Problem 8
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        def double(place):
          if not isinstance(place.ant, QueenAnt):
            if place.ant != None and not place.ant.doubled:
              place.ant.doubled = True
              place.ant.damage *= 2
            if place.ant != None and place.ant.container and place.ant.ant != None and not place.ant.ant.doubled:
              if not isinstance(place.ant.ant, QueenAnt):
                place.ant.ant.doubled = True
                place.ant.ant.damage *= 2


        exits = self.place
        while exits.exit != None:
          exits = exits.exit
          double(exits)
    ...
    def reduce_armor(self, amount):
        ...
        if not self.imposter and self.armor-amount <= 0:
          bees_win()
