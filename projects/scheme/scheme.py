...
def eval_all(expressions, env):
    ...
    to_do = expressions
    while to_do.second is not nil:
        scheme_eval(to_do.first, env)
        to_do = to_do.second

        
    return scheme_eval(to_do.first, env, True)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            if (vals is nil):
                raise SchemeError("# of args do not match")
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        result = operands.map(lambda operand: scheme_eval(operand, env))
        return self.apply(result, env)
        # END PROBLEM 5
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        return scheme_eval(complete_eval(self.apply(operands,env)),env)
        # END Problem 21
...
def do_define_form(expressions, env):
    ...
    target_expr = expressions.first
    if scheme_symbolp(target_expr):
        check_form(expressions, 2, 2)
        # BEGIN PROBLEM 6
        value = scheme_eval(expressions.second.first, env)
        env.define(target_expr, value)
        return target_expr
        # END PROBLEM 6
    elif isinstance(target_expr, Pair) and scheme_symbolp(target_expr.first):
        # BEGIN PROBLEM 10
        function = LambdaProcedure(target_expr.second, expressions.second, env)
        env.define(target_expr.first, function)
        return target_expr.first
        # END PROBLEM 10
    else:
        bad_target_expr = target_expr.first if isinstance(target_expr, Pair) else target_expr
        raise SchemeError('non-symbol: {0}'.format(bad_target_expr))
...
def do_and_form(expressions, env):
    ...
    while curr.second is not nil:
        result = scheme_eval(curr.first, env, False)
...
def make_let_frame(bindings, env):
    ...
    while curr is not nil:
        check_form(curr.first, 2, 2)
        if not scheme_symbolp(curr.first.first):
            raise SchemeError("Improper input")

        result = scheme_eval(curr.first.second.first, child.parent)
        child.define(curr.first.first, result)
        curr = curr.second
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target_expr):
        ...
        env.define(target_expr, result)
        return target_expr
    
    elif scheme_symbolp(target_expr.first) and isinstance(target_expr, Pair) :
        symbol = target_expr.first
        params = target_expr.second
        check_formals(params)
        
        body = expressions.second
        procedure = MacroProcedure(params,body,env)
