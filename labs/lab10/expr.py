...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        """
        try:
            value = env[self.string]
            return value
        except (KeyError, NameError):
            raise NameError(self.string, 'does not exist!')
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        primFunc = self.operator.eval(env)
        values = []
        for x in self.operands:
            if type(x) != Literal or type(x) != Number:
                values.append(x.eval(env))
            else:
                values.append(x)
        return primFunc.apply(values)
