...
def distance(city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    city1x = get_lat(city1)
    city1y = get_lon(city1)
    city2x = get_lat(city2)
    city2y = get_lon(city2)

    return sqrt((city1x - city2x)**2 + (city1y - city2y)**2)
...
def replace_elem(lst, index, elem):
    ...
    "*** YOUR CODE HERE ***"
    return [elem if x == index else lst[x] for x in range(len(lst))]
...
def put_piece(board, max_rows, column, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    for x in range(max_rows - 1, -1, -1):
        if get_piece(board, x, column) == '-':
            return [x, replace_elem(board, x, replace_elem(board[x], column, player))]
