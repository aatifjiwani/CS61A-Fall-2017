...
def skip(r):
    if r is Link.empty or r.rest is Link.empty:
            return r
    else:
        if r.first == r.rest.first:
            r.rest = r.rest.rest
            skip(r)
        else:
            skip(r.rest)
...
def remove_all(link , value):
    ...
    if link is Link.empty or link.rest is Link.empty:
        ...
        rest = remove_all(link.rest, value)
...
def reverse(lnk):
    if lnk.rest is Link.empty:
        ...
        rest = reverse(lnk.rest)
        return Link(lnk.rest, rest)
...
def splice(a, b, k):
    if b is Link.empty:
        ...
        return Link(b.first, splice(a, b.rest, k))
    else:
        return Link(a.first, splice(a.rest, b, k-1))
...
def contains(a, b):
    if a is Link.empty or b is Link.empty:
        ...
        a, b = b, a
    return a.first == b.first or contains(a, b.rest)
...
class Path:
    def __init__(self, d, leaf_label):
        self.pile, self.end = d, leaf_label
