...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        years = 0
        compound = self.balance
        while compound < amount:
            years+=1
            compound *= (1 + self.interest)
        return years
...
def balanced(m):
    ...
    is_balance = False
    for side in sides(m):
        weights.append(length(side) * total_weight(end(side)))
    ...
    for side in sides(m):
        if not is_weight(end(side)):
            is_balance = is_balance and balanced(end(side))
...
def make_fib():
    ...
    """
    "*** YOUR CODE HERE ***"
    first, current, second = 0, 0, 1
    def fib():
        nonlocal first, current, second
        first = current
        current = second
        second = second + first
        return first
...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    "*** YOUR CODE HERE ***"
    correct_password = withdraw(0,old_password)
    if type(correct_password)==str:
